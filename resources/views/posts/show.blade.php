    ...
    <div class="md:w-1/2">
      ...
      <div>
        ...
        <p class="text-sm text-gray-500">
          {{-- La libreria Difference for Humans la encontramos en carbon laravel al instala por nosotros  --}}
      ...
      @auth {{-- reviso que este authenticado --}}
        @if ($post->user_id === auth()->user()->id)
          <form action="{{ route('posts.destroy', $post)}}" method="POST" >
            @method('DELETE')
            @csrf
           {{-- METHOdS SPOOFING: El navegador soporta nativamente 
              solo post y get pero spoofing nos permite agraegar metodos 
              como delete, put y patch --}}
            <input 
              type="submit" 
